```typescript
// --- START: Data Experience View ---
// This section contains the complete code for the "Data Experience" page and its related components.
...
const AddFieldModal = ({ isOpen, onClose, onConfirm, existingKeys }: { isOpen: boolean, onClose: () => void, onConfirm: (field: FieldDefinition) => void, existingKeys: string[] }) => {
    ...
    const handleLabelChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setLabel(newLabel);
        // A very basic pinyin-like transformation
...
const DataExperienceView = ({ factTables, schemas, onUpdateSchema, onClearTable, addToast }: any) => {
    ...
    const handleResetSchema = () => {
        if (window.confirm(`确定要将 [${selectedSchemaType}] 的表结构重置为默认设置吗？此操作会覆盖您的自定义修改。`)) {
            ...
            addToast('success', '重置成功', `[${selectedSchemaType}] 表结构已恢复为默认值。`);
        }
    };
    ...
    const handleDownloadTemplate = () => {
        try {
            const headers = currentSchema.map(field => field.label);
            const ws = XLSX.utils.aoa_to_sheet([headers]);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "模板");
            XLSX.writeFile(wb, `${selectedSchemaType}_template.xlsx`);
    ...
    const handleDeleteData = () => {
        if (window.confirm(`确定要清空 [${selectedSchemaType}] 表的所有数据吗？此操作不可撤销，但表结构会保留。`)) {
                    ...
                    <div>
                     ...
                     <div className="flex bg-white rounded-xl p-1 shadow-sm border border-slate-100">
                        {['shangzhi', 'jingzhuntong', 'customer_service'].map((t) => (
                            ...
                            >
                                {t === 'shangzhi' ? '商智明细' : t === 'jingzhuntong' ? '广告明细' : '客服明细'}
                            </button>
                        ))}
                    </div>
                </div>
                        ...
                        <div className="p-8 bg-slate-50/30 flex-1">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="font-bold text-slate-700 flex items-center gap-2">
                                    ...
                                    {selectedSchemaType === 'shangzhi' ? '商智' : selectedSchemaType === 'jingzhuntong' ? '京准通' : '客服'} 字段映射定义
                                </h3>
                            ...
                            <div className="space-y-4">
                                {sortedSchema.map((field: FieldDefinition, idx: number) => (
                                    <div key={idx} className="bg-white border border-slate-100 rounded-2xl p-6 flex items-start justify-between shadow-sm hover:shadow-md transition-shadow group">
                                        ...
                                        <button className="text-slate-300 hover:text-[#70AD47] transition-colors p-2 opacity-0 group-hover:opacity-100">
                                            <Edit2 size={16} />
                                        </button>
                                    </div>
                                ))}
                        ...
                        <div className="flex-1 flex flex-col">
                            <div className="p-4 border-b border-slate-100 flex gap-4 bg-slate-50/50">
                                 <div className="relative flex-1">
                                    ...
                                    <input placeholder="检索全表字段..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm outline-none focus:border-[#70AD47]" />
                                 </div>
                            ...
                            <div className="flex-1 overflow-auto">
                                <table className="w-full text-left text-xs whitespace-nowrap">
                                    <thead className="bg-slate-50 sticky top-0 z-10">
                                        <tr>
                                            {currentSchema.map((f:FieldDefinition) => (
                                                <th key={f.key} className="px-6 py-4 font-bold text-slate-500 border-b border-slate-200">{f.label}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-100">
                                        {factTables[selectedSchemaType] && factTables[selectedSchemaType].length > 0 ? (
                                            factTables[selectedSchemaType].slice(0, 10).map((row: any, rIdx: number) => (
                                                <tr key={rIdx} className="hover:bg-slate-50">
                                                    {currentSchema.map((f:FieldDefinition) => (
                                                        <td key={f.key} className="px-6 py-3 text-slate-600">{row[f.key] || '-'}</td>
                                                    ))}
                                                </tr>
                                            ))
                                        ) : (
                                            <tr>
                                                <td colSpan={currentSchema.length} className="py-32 text-center">

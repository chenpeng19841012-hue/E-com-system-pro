...
const DiagnosisCard: React.FC<{ d: Diagnosis, mode?: 'carousel' | 'list', onClickMore?: () => void }> = ({ d, mode = 'carousel', onClickMore }) => {
    // 首页轮播模式下，强制限制显示 3 条详情，且高度固定
    const detailEntries = Object.entries(d.details);
    const limit = mode === 'carousel' ? 3 : 100;
    const visibleDetails = detailEntries.slice(0, limit);
    const hiddenCount = detailEntries.length - limit;

    return (
        <div className={`transition-all duration-700 w-full flex flex-col ${mode === 'carousel' ? 'h-[190px] p-5 rounded-[20px] border border-slate-100 bg-white shadow-sm mb-4' : 'p-6 rounded-[24px] border border-slate-100 bg-slate-50/50 hover:bg-white hover:shadow-xl'}`}>
            <div className="flex items-center gap-3 mb-2 shrink-0">
                <div className={`p-1.5 rounded-lg ${d.severity === 'critical' ? 'bg-rose-100 text-rose-600' : d.severity === 'warning' ? 'bg-amber-100 text-amber-600' : 'bg-brand/10 text-brand'}`}>
                    {d.type === 'new_sku' ? <PackageSearch size={16}/> :
                     d.type === 'stock_severe' ? <AlertTriangle size={16}/> :
                     d.type === 'low_roi' ? <TrendingDown size={16}/> :
                     d.type === 'high_potential' ? <Zap size={16}/> :
                     d.type === 'stale_inventory' ? <Layers size={16}/> :
                     d.severity === 'critical' ? <ShieldAlert size={16}/> : 
                     <Flame size={16}/>}
                </div>
                <h4 className={`text-sm font-black uppercase tracking-tight truncate ${d.severity === 'critical' ? 'text-rose-600' : d.severity === 'warning' ? 'text-amber-600' : 'text-slate-800'}`}>{d.title}</h4>
            </div>
            <p className="text-[10px] font-bold text-slate-400 leading-relaxed mb-3 line-clamp-1 shrink-0">{d.desc}</p>
            
            <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 space-y-2 overflow-hidden flex-1 relative">
                {visibleDetails.map(([k,v]) => (
                    <div key={k} className="flex flex-col gap-0.5 text-[9px] font-bold border-b border-slate-100 last:border-0 pb-1 last:pb-0">
                        {/* 如果 key 是特定的通用描述（如“风险列表”），我们只显示 value，否则显示 key: value */}
                        <span className="text-slate-700 leading-relaxed break-all font-mono whitespace-pre-wrap">{v}</span>
                    </div>
                ))}
                {mode === 'carousel' && hiddenCount > 0 && (
                    <div onClick={onClickMore} className="absolute bottom-0 left-0 w-full bg-slate-50/95 py-1 text-center cursor-pointer hover:bg-slate-100 transition-colors border-t border-slate-200">
                        <p className="text-[9px] font-black text-brand">... 以及其他 {hiddenCount} 个 (点击查看)</p>
                    </div>
                )}
            </div>
        </div>
...
const MainTrendVisual = ({ data, metricKey }: { data: DailyRecord[], metricKey: MetricKey }) => {
    ...
    // 高度恢复至 320，容器 min-h 500px
    const width = 1000; const height = 320; const padding = { top: 30, right: 30, bottom: 40, left: 50 };
...
export const DashboardView = ({ skus, shops, factStats, addToast }: { skus: ProductSKU[], shops: Shop[], factStats?: any, addToast: any }) => {
    ...
    // 轮播状态：使用 translateY 实现滚动，每次展示 2 个
    ...
    useEffect(() => {
        ...
        const timer = setInterval(() => {
            setDiagOffset(prev => (prev + 1) % diagnoses.length);
        }, 5000);
        ...
        try {
            ...
            if (newlyActive.length > 0) {
                diag.push({
                    ...
                    details: { '激活清单': newlyActive.map(c => getSkuInfoStr(c!)).join('\n') }
                });
            }
            // 2. 增长失速
            ...
            if (stockRisks.length > 0) {
                diag.push({ 
                    ...
                    details: { '风险列表': stockRisks.map(s => {
                        const shopName = shopMap.get(s.shopId)?.name || '未知';
                        return `• ${s.name} (余:${(s.warehouseStock||0)+(s.factoryStock||0)})\n  [${shopName}] | ${s.model || '-'} | ${s.mode}`;
                    }).join('\n') } 
                });
            }
            // 4. 投放赤字
    ...
    return (
        <div className="p-8 md:p-12 w-full animate-fadeIn space-y-8 min-h-screen bg-[#F8FAFC]">
            ...
            {/* Main Section - Synced Height */}
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                {/* 增长拓扑流 - 恢复高度 */}
                <div className="xl:col-span-8 bg-white rounded-[40px] p-8 shadow-sm border border-slate-100 flex flex-col relative overflow-hidden group/chart min-h-[500px]">
                ...
                {/* AI 诊断室 - 同步高度，双卡片轮播 */}
                <div className="xl:col-span-4 bg-white rounded-[40px] p-8 shadow-xl border border-slate-100 flex flex-col relative overflow-hidden group/diag min-h-[500px]">
                    ...
                    {/* 垂直轮播容器：固定 400px，展示 2 个卡片 */}
                    <div className="h-[400px] relative mb-6 overflow-hidden">
                        {diagnoses.length === 0 ? (
                            ...
                            <div 
                                ...
                                style={{ transform: `translateY(-${(diagOffset * 206)}px)` }} // 190px height + 16px margin-bottom = ~206px per item scroll unit? No, let's keep it simpler by just rendering specific items if possible, but translate is smoother.
                                // Better approach: Render logic instead of raw scroll to allow infinite loop effect visually or just scroll down
                            >
                                {/* 为了实现无缝循环，这里使用更简单的渲染逻辑：只渲染当前窗口的2-3个项目，或者直接渲染列表并调整 top */}
                                {/* 简化方案：直接显示列表，通过 style top 移动 */}
                                <div className="flex flex-col">
                                    {diagnoses.map((d, i) => (
                                        // 仅当 index 在 display window 内或附近时渲染，或者全部渲染依靠 overflow hidden
                                        <div key={d.id} className="h-[190px] mb-4 shrink-0">
                                            <DiagnosisCard d={d} mode="carousel" onClickMore={() => setIsAllDiagnosesModalOpen(true)} />
                                        </div>
                                    ))}
                                    {/* 重复第一二个以支持无缝滚动的视觉骗局？这里直接采用回滚方式 */}
                                </div>
                            </div>
                        )}
                        {/* 覆盖层：如果列表很长，且不在滚动，指示还有更多 */}
                        {diagnoses.length > 2 && (
                            <div className="absolute bottom-0 left-0 w-full h-12 bg-gradient-to-t from-white to-transparent pointer-events-none flex items-end justify-center">
                                <div className="w-1.5 h-1.5 bg-slate-200 rounded-full mb-2 animate-bounce"></div>
                            </div>
                        )}
                    </div>
